...
(function(){
  ...
  CMS.Forms.number = Backbone.View.extend({
    tmpl: CMS.Templates.number,

    getValue: function () {
      var str = this.$('#' + this.model.key).val();
      var value = (str) ? Number(str) : undefined; // Number()に空文字を入れると0になり、requireチェックが出来ない為注意
      if (_.isNaN(value)) { value = str; }
      return value;
    },
